...
public class CommentsController(ICommentRepository commentRepository, IUserRepository userRepository) : ControllerBase {
    ...
    public async Task<ActionResult<CommentDto>> AddComment([FromBody] CreateCommentDto request) {
        ...
        CommentDto commentDto = new() {
            ...
            AuthorUsername = userRepository.GetSingleAsync(createdComment.UserId).Result.Username
        };
    ...
    public ActionResult<IEnumerable<CommentDto>> GetComments([FromQuery] int? postId) {
        ...
        IEnumerable<CommentDto> commentDtos = comments.Select(comment => new CommentDto {
            Id = comment.Id,
            Body = comment.Body,
            PostId = comment.PostId,
            AuthorUsername = userRepository.GetSingleAsync(comment.UserId).Result.Username
        });
